...
function init(page) {
    ...
    request.onreadystatechange = function () {
      ...
      for(let i = 0; i < data.length; i++){
          htmlContent += `<li><a href ="detalles.html?${data[i].id}">, <img src = ${data[i].picture} width = 80px>, ${data[i].firstName}, ${data[i].lastName}</li>`; // il fuat remettre le meme nom que la constante
      }
   htmlContent += "</ul>"
  ...
  request.open("GET", `https://dummyapi.io/data/v1/user?limit=10&page=${page}", true`) // on appelle en haut
